...
from ropendb.settings import RO_JOBS, RO_ARMOR_LOCATIONS
...
class ArmorSerializer(serializers.HyperlinkedModelSerializer):
    ...
    class Meta:
        ...
        fields = ['id', 'name_japanese', 'type', 'subtype', 'price_buy', 'price_sell', 'weight', 'defence',
                  'equip_level_min', 'slots', 'equip_jobs', 'script', 'lootedFrom', 'equip_locations']
...
class ArmorViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def get_queryset(self):
        search_fields = ['equip_locations', 'slots', 'id', 'name_english']
        params = self.request.query_params
        queryset = ItemDb.objects.filter(type=5)
